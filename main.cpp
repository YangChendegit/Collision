...
int main(int argc, char *argv[]) {
  ...
  Abstract2<DATA_TYPE, COUNT_TYPE>*  HH_PP = new Hyper<DATA_TYPE, COUNT_TYPE, 3, 3, 1>(mem/1024  , 0.6, hit > 300 ? 255 : hit - 25 - 20, 25, 85, window_size); 
   collision *dds = new collision(mem,hh, window_size, hit*1.0/(ttt1.size()/window_size));
  ...
  initial = now();
   ...
   finish = now();
   std::cout << "Insertion Thp of Collision:" << ttt1.size() / durationms(finish, initial) << std::endl;
